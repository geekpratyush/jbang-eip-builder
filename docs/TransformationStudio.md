...
> - **XSLT**: `to: "xslt-saxon:file:transform.xslt"`
...
## 🚀 Remote File Copying (Data Transformation & Sandbox Paths)

When working with a remote Camel Quarkus testing container, you can copy your transformation mappings directly to the server's filesystem using the **Copy to Remote** toolbar action.

### 1. Folder Hierarchy on the Server
To keep maps organized, the client automatically nests the uploaded files under the target base directory:
```
<server_target_path>/
  └── <transformer_type>/
        └── <id>/
...
*   **XML Parser Map:**
    ...
    *   **Type:** `xml`
    *   **File:** `definition.xml`
    *   **Resulting Server Path:** `/opt/camel/resources/xml/example2/definition.xml`
*   **XSLT Pacs008 Map:**
    *   **ID:** `pacs008`
    *   **Type:** `xslt`
    *   **File:** `enrich.xslt`
    *   **Resulting Server Path:** `/opt/camel/resources/xslt/pacs008/enrich.xslt`

### 2. Copy/Paste into Camel Route
When the copy is successful, the absolute path is printed in the deployment window's logs. Copy this path and use it directly inside your route endpoint definitions:
```yaml
- from:
    uri: "direct:start"
    steps:
      - to: "xslt-saxon:file:/opt/camel/resources/xslt/pacs008/enrich.xslt"
```
