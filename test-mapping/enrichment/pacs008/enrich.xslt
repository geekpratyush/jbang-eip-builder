...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:p="urn:iso:std:iso:20022:tech:xsd:pacs.008.001.14" exclude-result-prefixes="p">
...
<xsl:template match="/*[local-name()='envelope']">
  ...
  <xsl:apply-templates select="$mergedTree" mode="clean"/>
</xsl:template>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:for-each select="*">
          ...
          <xsl:variable name="pos" select="count(preceding-sibling::*[local-name() = $nodeName]) + 1"/>
...
<xsl:template match="node()" mode="clean"><xsl:copy><xsl:apply-templates select="node()" mode="clean\"/></xsl:copy></xsl:template>
</xsl:stylesheet>
