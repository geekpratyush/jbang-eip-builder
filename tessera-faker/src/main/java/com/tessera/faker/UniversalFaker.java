...
import com.tessera.faker.generators.*;
...
public class UniversalFaker {
    ...
    private final GenerationContext globalContext = new GenerationContext();
    ...
    private void initialize(Path templatesPath, Path dbPath) {
        synchronized (this) {
            this.discovery = new TemplateDiscovery(templatesPath);
    ...
    private static Path findTemplatesPath() {
        // 1. Check Env Vars / System Properties (New Standard)
        String env = System.getenv("FAKER_TEMPLATES_DIR");
        if (env == null || env.isEmpty()) env = System.getProperty("FAKER_TEMPLATES_DIR");
        if (env != null && !env.isEmpty()) return Paths.get(env);

        // 2. Check Legacy / Internal Properties
        env = System.getenv("TESSERA_FAKER_TEMPLATES");
        if (env == null || env.isEmpty()) env = System.getProperty("tessera.faker.templates");
        if (env != null && !env.isEmpty()) return Paths.get(env);
        
        // 3. Check Workspace Root Context
        String wsRoot = System.getenv("WORKSPACE_ROOT_DIR");
        if (wsRoot == null || wsRoot.isEmpty()) wsRoot = System.getProperty("WORKSPACE_ROOT_DIR");
        if (wsRoot != null && !wsRoot.isEmpty()) {
            Path p = Paths.get(wsRoot).resolve("FAKER").resolve("templates");
            if (java.nio.file.Files.exists(p)) return p;
        }

        // 4. Walk up the tree (Automatic Discovery)
        Path current = Paths.get(System.getProperty("user.dir"));
        while (current != null) {
            Path p = current.resolve("FAKER").resolve("templates");
            if (java.nio.file.Files.exists(p)) return p;
            p = current.resolve("templates");
            if (java.nio.file.Files.exists(p)) return p;
            current = current.getParent();
        }
        return Paths.get(System.getProperty("user.dir"), "templates");
    }

    private static Path findDefaultDbPath() {
        // 1. Check Env Vars / System Properties (New Standard)
        String env = System.getenv("FAKER_DB_DIR");
        if (env == null || env.isEmpty()) env = System.getProperty("FAKER_DB_DIR");
        if (env != null && !env.isEmpty()) return Paths.get(env);

        // 2. Check Legacy / Internal Properties
        env = System.getenv("TESSERA_FAKER_DB");
        if (env == null || env.isEmpty()) env = System.getProperty("tessera.faker.db");
        if (env != null && !env.isEmpty()) return Paths.get(env);
        ...
        if (wsRoot == null || wsRoot.isEmpty()) wsRoot = System.getProperty("WORKSPACE_ROOT_DIR");
        if (wsRoot != null && !wsRoot.isEmpty()) {
            Path p = Paths.get(wsRoot).resolve("FAKER").resolve("faker-db");
        ...
        Path current = Paths.get(System.getProperty("user.dir"));
        while (current != null) {
            Path p = current.resolve("FAKER").resolve("faker-db");
            if (java.nio.file.Files.exists(p)) return p.resolve("financial");
            p = current.resolve("faker-db");
            if (java.nio.file.Files.exists(p)) return p.resolve("financial");
            current = current.getParent();
        }
        return null;
    ...
    /**
     * Standard setter for bean property "templatesDir"
     */
    ...
    /**
     * Standard setter for bean property "dbDir"
     */
    ...
    private static Path findWorkspaceRoot() {
        Path current = Paths.get(System.getProperty("user.dir"));
        while (current != null) {
            if (java.nio.file.Files.exists(current.resolve("FAKER")) || 
                java.nio.file.Files.exists(current.resolve("application.properties"))) {
                return current;
            }
            current = current.getParent();
        }
    ...
    public void setDatabase(Path targetDb) {
        synchronized (this) {
            this.dbService = new DatabaseService(targetDb);
