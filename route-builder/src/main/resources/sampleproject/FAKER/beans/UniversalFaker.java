...
public class UniversalFaker {
    ...
    public UniversalFaker() {
        Path baseDb = findPath("faker-db");
        Path financialDb = baseDb.resolve("financial");
        if (Files.exists(financialDb)) {
            initialize(findPath("templates"), financialDb);
        } else {
            initialize(findPath("templates"), baseDb);
        }
    }
    ...
    public void initialize(Path templatesDir, Path dbDir) {
        synchronized (UniversalFaker.class) {
            if (!initialized) {
                discovery = new TemplateDiscovery(templatesDir);
                dbService = new DatabaseService(dbDir);
                engine = new TemplateEngine();
                registerDefaultGenerators();
                discovery.scanTemplates();
                initialized = true;
            } else {
                if (templatesDir != null && !templatesDir.equals(discovery.getTemplatesDir())) {
                    discovery = new TemplateDiscovery(templatesDir);
                    discovery.scanTemplates();
                }
                if (dbDir != null && !dbDir.equals(dbService.getDbDir())) {
                    dbService = new DatabaseService(dbDir);
                }
            }
        }
    ...
    public void setDatabase(String dbName) {
        Path baseDb = findPath("faker-db");
        Path targetDb = baseDb.resolve(dbName);
    ...
    private static Path findPath(String dirName) {
        String userDir = System.getProperty("user.dir");
        
        Path p = Paths.get(userDir, dirName);
        if (Files.exists(p)) return p;
        
        p = Paths.get(userDir, "FAKER", dirName);
        if (Files.exists(p)) return p;

        p = Paths.get(userDir, "..", dirName);
        if (Files.exists(p)) return p;

        return Paths.get(userDir, dirName);
    }
    ...
    public String generate(String messageType, Map<String, Object> overrides) {
        ...
        if (messageType.contains(",")) {
            String[] types = messageType.split(",");
            String chosen = types[new Random().nextInt(types.length)].trim();
            return generateSingle(chosen, overrides);
        }
        return generateSingle(messageType, overrides);
    }

    private String generateSingle(String messageType, Map<String, Object> overrides) {
        String template = discovery.getTemplate(messageType);
        if (template == null) {
            return "No template found for: " + messageType;
        }
        GenerationContext ctx = globalContext.fork();
        ctx.setOverrides(overrides);
        return engine.process(template, ctx, dbService);
    }
